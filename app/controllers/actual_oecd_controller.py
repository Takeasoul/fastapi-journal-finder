...
@router.get("/", response_model=list[ActualOECDOut],  dependencies=[Depends(require_role("user"))])
...
@router.get("/{id}", response_model=ActualOECDOut,  dependencies=[Depends(require_role("user"))])
async def get_actual_oecd(actual_oecd_id: int, db: AsyncSession = Depends(get_db1_session)):
    ...
    if not record:
        raise HTTPException(status_code=404, detail="Actual OECD not found")
    return record

@router.post("/", response_model=ActualOECDOut,  dependencies=[Depends(require_role("admin"))])
...
@router.put("/{id}", response_model=ActualOECDOut,  dependencies=[Depends(require_role("admin"))])
...
async def delete_actual_oecd(actual_oecd_id: int, db: AsyncSession = Depends(get_db1_session)):
    ...
    if not success:
        raise HTTPException(status_code=404, detail="Actual OECD not found")
    return {"detail": "Actual OECD deleted"}
