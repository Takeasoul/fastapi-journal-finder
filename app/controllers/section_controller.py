...
@router.get("/", response_model=list[SectionOut],  dependencies=[Depends(require_role("user"))])
async def list_sections(db: AsyncSession = Depends(get_db1_session)):
    return await section_service.get_all_sections(db)

@router.get("/{section_id}", response_model=SectionOut,  dependencies=[Depends(require_role("user"))])
...
@router.post("/", response_model=SectionOut,  dependencies=[Depends(require_role("admin"))])
...
@router.put("/{section_id}", response_model=SectionOut,  dependencies=[Depends(require_role("admin"))])
