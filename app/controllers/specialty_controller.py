...
@router.get("/", response_model=list[SpecialtyOut],  dependencies=[Depends(require_role("user"))])
async def list_specialties(db: AsyncSession = Depends(get_db1_session)):
    return await specialty_service.get_all_specialties(db)

@router.get("/{specialty_id}", response_model=SpecialtyOut,  dependencies=[Depends(require_role("user"))])
...
@router.put("/{specialty_id}", response_model=SpecialtyOut,  dependencies=[Depends(require_role("admin"))])
